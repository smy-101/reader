...
- [ ] 两表经 Flyway 迁移落地;外键级联删除;updated_at 服务器时钟,客户端时间戳不参与裁决(D-19)
- [ ] 划线:按书全量拉取 / 创建 / 单条改颜色备注 / 单条删除,CFI、文字快照、颜色、备注、设备标识落库(D-24)
- [ ] 进度:单行读取 + 单条 upsert,重复 upsert 覆盖不产生多行
- [ ] LWW:同一划线两次更新,后写胜(断言最终值为后写);无合并 UI
- [ ] 书库列表返回真实进度百分比,无进度的书该字段为空(M0 占位转实)
- [ ] 新端点无/错 token 一律 401;Seam A 集成测试覆盖上述行为,`mvn test` 全绿
